...
func TestNewSolanaEngine(t *testing.T) {
	...
	t.Run("create new solana engine", func(t *testing.T) {
		_, err := gojup.NewSolanaEngine(
			wallet,
			"http://localhost:8899",
			gojup.WithMaxRetries(10),
		)
		require.NoError(t, err)
	})
	...
	t.Run("solana engine with rpc endpoint", func(t *testing.T) {
		_, err := gojup.NewSolanaEngine(
			wallet,
			"",
			gojup.WithSolanaClientRPC(rpcMock{}),
		)
		require.NoError(t, err)
	})

	t.Run("execute valid swap", func(t *testing.T) {
		eng, err := gojup.NewSolanaEngine(
			wallet,
			"",
			gojup.WithSolanaClientRPC(rpcMock{}),
		)
		...
		expectedTxID := gojup.TxID("24jRjMP3medE9iMqVSPRbkwfe9GdPmLfeftKPuwRHZdYTZJ6UyzNMGGKo4BHrTu2zVj4CgFF3CEuzS79QXUo2CMC")
	...
	t.Run("error when getting the blockhash", func(t *testing.T) {
		eng, err := gojup.NewSolanaEngine(
			wallet,
			"",
			gojup.WithSolanaClientRPC(rpcMock{shouldFailGetLatestBlockhash: true}),
		)
		require.NoError(t, err)
	...
	t.Run("error when sending the transaction on chain", func(t *testing.T) {
		eng, err := gojup.NewSolanaEngine(
			wallet,
			"",
			gojup.WithSolanaClientRPC(rpcMock{shouldFailSendTransaction: true}),
		)
		require.NoError(t, err)
